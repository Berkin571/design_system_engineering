...
function getCycles(dependencies) {
  ...
  function dfs(next, path10) {
    visited.add(next);
    path10.push(next);
    for (const dep of dependencies[next] || []) {
      if (path10.includes(dep)) {
        const cycle = path10.slice(path10.indexOf(dep));
        cycles.push(cycle);
...
var import_assert, import_fs4, import_path11, ConfigChain, Conf;
var init_conf = __esm({
  "libs/core/src/lib/npm-conf/conf.ts"() {
    "use strict";
...
function npmRunScript(script, { args, npmClient, pkg, reject = true }) {
  import_npmlog26.default.silly("npmRunScript", script, args, pkg.name);
...
function npmRunScriptStreaming(script, { args, npmClient, pkg, prefix, reject = true }) {
  import_npmlog26.default.silly("npmRunScriptStreaming", [script, args, pkg.name]);
  const argv = ["run", script, ...args];
  const opts = makeOpts(pkg, reject);
...
var init_profiler = __esm({
  "libs/core/src/lib/profiler.ts"() {
    ...
    Profiler = class {
      ...
      outputPath;
      threads;
      constructor({ concurrency, log: log34 = import_npmlog29.default, outputDirectory }) {
        this.events = [];
        this.logger = log34;
  ...
  "libs/commands/run/src/index.ts"(exports2, module2) {
    ...
    var RunCommand = class extends Command {
      ...
      runScriptInPackagesParallel() {
        return (0, import_p_map3.default)(this.projectsWithScript, (p) => this.runScriptInPackageStreaming(getPackage(p)));
      }
      runScriptInPackagesLexical() {
        return (0, import_p_map3.default)(this.projectsWithScript, (p) => this.getRunner()(getPackage(p)), {
          concurrency: this.concurrency
        });
      }
      ...
      async runScriptsUsingNx() {
        ...
        if (this.options.profile) {
          ...
          process.env.NX_PROFILE = import_path13.default.relative(this.project.rootPath, absolutePath);
        }
      ...
      async prepNxOptions() {
        const nxJsonExists = (0, import_fs_extra8.existsSync)(import_path13.default.join(this.project.rootPath, "nx.json"));
