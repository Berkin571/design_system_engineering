...
var import_npmlog9, import_os, import_path6, import_write_file_atomic;
...
var import_clone_deep, import_dedent3, import_npmlog12, import_client, import_os2, DEFAULT_CONCURRENCY, Command;
...
function getCurrentBranch(opts) {
  import_npmlog36.default.silly("getCurrentBranch");
  const branch = childProcess10.execSync("git", ["rev-parse", "--abbrev-ref", "HEAD"], opts);
  import_npmlog36.default.verbose("currentBranch", branch);
  return branch;
}
...
function resolvePrettier() {
  if (!resolvedPrettier) {
    try {
      const packageJson = (0, import_devkit5.readJsonFile)(import_path18.default.join(import_devkit5.workspaceRoot, "package.json"));
...
async function maybeFormatFile(filePath) {
  ...
  const ignorePath = import_path18.default.join(import_devkit5.workspaceRoot, ".prettierignore");
  const fullFilePath = import_path18.default.join(import_devkit5.workspaceRoot, filePath);
  const fileInfo = await resolvedPrettier.getFileInfo(fullFilePath, { ignorePath });
  if (fileInfo.ignored) {
    import_npmlog37.default.silly("version", `Skipped applying prettier to ignored file: ${filePath}`);
    return;
  }
  try {
    ...
    import_npmlog37.default.silly("version", `Successfully applied prettier to updated file: ${filePath}`);
  } catch {
    import_npmlog37.default.silly("version", `Failed to apply prettier to updated file: ${filePath}`);
  }
}
async function gitAdd(changedFiles, gitOpts, execOpts) {
  let files = [];
  for (const file of changedFiles) {
    const filePath = (0, import_slash2.default)(import_path18.default.relative(execOpts.cwd, import_path18.default.resolve(execOpts.cwd, file)));
  ...
  import_npmlog37.default.silly("gitAdd", files);
  return childProcess11.exec("git", ["add", "--", ...files], execOpts);
}
var import_devkit5, import_fs5, import_npmlog37, import_path18, import_slash2, childProcess11, resolvedPrettier;
...
var import_npmlog38, import_os3, childProcess12;
...
function gitPush(remote, branch, opts) {
  import_npmlog39.default.silly("gitPush", remote, branch);
  return childProcess13.exec("git", ["push", "--follow-tags", "--no-verify", "--atomic", remote, branch], opts).catch((error) => {
    if (/atomic/.test(error.stderr) || process.env.GIT_REDIRECT_STDERR === "2>&1" && /atomic/.test(error.stdout)) {
      import_npmlog39.default.warn("gitPush", error.stderr);
      import_npmlog39.default.info("gitPush", "--atomic failed, attempting non-atomic push");
...
function gitTag(tag, { forceGitTag, signGitTag }, opts, command = "git tag %s -m %s") {
  import_npmlog40.default.silly("gitTag", tag, command);
  ...
  import_npmlog40.default.verbose(cmd, interpolatedArgs);
  return childProcess14.exec(cmd, interpolatedArgs, opts);
}
var import_npmlog40, childProcess14;
...
function isAnythingCommitted(opts) {
  import_npmlog41.default.silly("isAnythingCommitted", "");
  const anyCommits = childProcess15.execSync("git", ["rev-list", "--count", "--all", "--max-count=1"], opts);
  import_npmlog41.default.verbose("isAnythingCommitted", anyCommits);
  return Boolean(parseInt(anyCommits, 10));
}
var import_npmlog41, childProcess15;
...
var import_npmlog42, childProcess16;
...
function remoteBranchExists(gitRemote, branch, opts) {
  import_npmlog43.default.silly("remoteBranchExists", "");
  const remoteBranch = `${gitRemote}/${branch}`;
  try {
...
var import_npmlog43, childProcess17;
...
async function updateLockfileVersion(pkg) {
  const lockfilePath = import_path19.default.join(pkg.location, "package-lock.json");
  const obj = await (0, import_load_json_file3.default)(lockfilePath).catch(() => {
    import_npmlog44.default.verbose("version", `${pkg.name} has no lockfile. Skipping lockfile update.`);
  });
  if (!obj) {
    return;
  }
...
var import_devkit6, import_load_json_file3, import_npmlog44, import_path19;
...
var require_src = __commonJS({
  "libs/commands/version/src/index.ts"(exports2, module2) {
    ...
    var childProcess21 = require("@lerna/child-process");
    ...
    var VersionCommand = class extends Command {
      ...
      async updatePackageVersions() {
        ...
        if (this.options.npmClient === "yarn") {
          const yarnVersion = await childProcess21.execSync("yarn", ["--version"], this.execOpts);
          ...
          if (import_semver8.default.gte(yarnVersion, "2.0.0")) {
            ...
            await childProcess21.exec("yarn", ["install", "--mode", "update-lockfile", ...npmClientArgs], {
              ...this.execOpts,
              env: {
                ...process.env,
      ...
      async hasChanges() {
        try {
          await (0, import_execa2.default)("git", ["diff", "--staged", "--quiet"], { stdio: "pipe", ...this.execOpts });
        } catch (e) {
          return true;
        }
      }
...
var require_command = __commonJS({
  "libs/commands/version/src/command.ts"(exports2, module2) {
    "use strict";
    var import_npmlog50 = __toESM(require("npmlog"));
    var import_semver8 = __toESM(require("semver"));
    ...
    var command = {
      ...
      builder(yargs2, composed) {
        ...
        return yargs2.option("ignore", {
          ...
          if (argv["--"]) {
            import_npmlog50.default.warn("EDOUBLEDASH", "Arguments after -- are no longer passed to subprocess executions.");
            import_npmlog50.default.warn("EDOUBLEDASH", "This will cause an error in a future major version.");
          }
          return argv;
        });
      },
...
function createTempLicenses(srcLicensePath, packagesToBeLicensed) {
  ...
  const licenseFileName = import_path20.default.basename(srcLicensePath);
  const options = {
  ...
  packagesToBeLicensed.forEach((pkg) => {
    pkg.licensePath = import_path20.default.join(pkg.contents, licenseFileName);
  });
...
function getCurrentSHA(opts) {
  import_npmlog45.default.silly("getCurrentSHA", "");
  const sha = childProcess18.execSync("git", ["rev-parse", "HEAD"], opts);
  import_npmlog45.default.verbose("getCurrentSHA", sha);
  return sha;
}
...
async function getCurrentTags(execOpts, matchingPattern) {
  import_npmlog46.default.silly("getCurrentTags", "matching %j", matchingPattern);
  const opts = Object.assign({}, execOpts, {
    // don't reject due to non-zero exit code when there are no results
    reject: false
  });
  const result = await childProcess19.exec(
    "git",
...
var import_npm_package_arg7, import_npmlog46, childProcess19;
...
function getPackagesWithoutLicense(project, packagesToPublish) {
  return project.getPackageLicensePaths().then((licensePaths) => {
    const licensed = new Set(licensePaths.map((lp) => import_path21.default.dirname(lp)));
...
async function getProjectsWithTaggedPackages(projectNodes, projectFileMap, execOpts) {
  import_npmlog48.default.silly("getTaggedPackages", "");
  const result = await childProcess20.exec(
    "git",
...
var import_npmlog48, childProcess20;
...
async function getProjectsWithUnpublishedPackages(projectNodes, opts) {
  import_npmlog49.default.silly("getProjectsWithUnpublishedPackages", "");
  const mapper = (node) => {
    const pkg = getPackage(node);
    ...
    return import_pacote.default.packument(pkg.name, opts).then(
      ...
      () => {
        import_npmlog49.default.warn("", "Unable to determine published version, assuming %j unpublished.", pkg.name);
        return node;
      }
    );
  };
...
var import_npmlog49, import_p_map4, import_pacote;
      ...
      builder(yargs2) {
        ...
        return yargs2.option("npm-tag", {
          ...
          if (argv["npmTag"]) {
            ...
            delete argv["npmTag"];
            delete argv["npm-tag"];
            import_npmlog50.default.warn("deprecated", "--npm-tag has been renamed --dist-tag");
          }
          return argv;
        }).middleware((args) => {
